...
# Public Subnet 2 (for Bastion - high availability)
...
# Private Subnet 1 (for Application VM)
...
# Security Group for Application VM
resource "aws_security_group" "app_vm" {
  ...
  ingress {
    description     = "HTTP from Load Balancer or Bastion"
    from_port       = 5000
    to_port         = 5000
    protocol        = "tcp"
...
# Security Group for Load Balancer (optional, for public access)
...
# Update App VM security group to allow traffic from ALB
resource "aws_security_group_rule" "app_vm_from_alb" {
  type                     = "ingress"
  from_port                = 5000
  to_port                  = 5000
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.alb.id
  security_group_id        = aws_security_group.app_vm.id
...
# Key Pair for EC2 instances (you should create this manually or use existing)
# For production, use AWS Systems Manager Parameter Store or Secrets Manager
# Note: This data source will fail if the key doesn't exist. Create it first:
